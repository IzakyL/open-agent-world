...
- Supported public surfaces: Conversation and its sections, Sandbox files/preview and explicitly allowed terminal, Text, Image, `oaw.tasks` task boards, and Agent status. Other plugin surfaces (including MatCreator-specific views) fail publication with an actionable error and may remain as hidden backend dependencies.
