...
const source = process.argv.includes('--store-real') ? 'store-real' : process.argv.includes('--store-fake') ? 'store-fake' : 'local';
...
const expectedSha = createHash('sha256').update(await readFile(build.artifact)).digest('hex');
...
try {
  ...
  const receipt = { status: 'passed', source, desktopPayload: payload ?? null, ...build, dataRoot, url, sha256: expectedSha };
